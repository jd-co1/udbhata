    ...
    if option == "drreddy" or option == "abbott" or option == "gsk" or option == "novartis" or option == "takeda" or option == "teva":
        ...
        if option == f'{option}':
            ...
            if st.checkbox(label="Show calculations",key=2):
                ...
                st.write(f"6% weightage of Ease of Doing business so 6% of {converted[8]} is",round((6/100)*(converted[8]),2))
                st.write(f"20% weightage of Economic Risk so 20% of {converted[9]} is",round((20/100)*(converted[9]),2))
                st.write(f"3% weightage of Competitiveness Index so 3% of {converted[10]} is",round((3/100)*(converted[10]),2))
                st.write(f"2% weightage of Global Terrorism Impact so 2% of {converted[11]} is",round((2/100)*(converted[10]),2))
